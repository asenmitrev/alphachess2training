...
class Config:
    ...
    num_residual_blocks: int = 3   # Deeper network
    
    # MCTS settings
    num_simulations: int = 100  # More simulations for better play
    c_puct: float = 2.0  # Increased for more exploration
    dirichlet_alpha: float = 0.3  # Dirichlet noise parameter for root node exploration
    dirichlet_epsilon: float = 0.25  # Mixing factor for Dirichlet noise (0.25 = 25% noise, 75% prior)
    ...
    num_epochs: int = 1  # Single pass to prevent overfitting
